 ...
 */
export function useColumnSettings<T>(tableId: string, columns: TableColumn<T>[], storageKey?: string) {
  ...
  useEffect(() => {
    try { localStorage.setItem(key, JSON.stringify(pref)); } catch { /* quota / private mode */ }
  }, [key, pref]);
  ...
  const { orderedColumns, visibleColumns } = useMemo(() => {
    ...
    for (const k of pref.order) if (byKey.has(k) && !orderedKeys.includes(k)) orderedKeys.push(k);
    for (const c of columns) if (!orderedKeys.includes(c.key)) orderedKeys.push(c.key);
  ...
  function seedWidths(seed: Record<string, number>) {
